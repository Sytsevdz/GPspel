...
export async function getGlobalPlayerGrandPrixView(
  ...
  return loadPlayerGrandPrixViewData(supabase, safeGrandPrixId, safePlayerId, safePlayerName);
      ...
      prediction.sprint_race_p2 ?? "",
      prediction.sprint_race_p3 ?? "",
    const sprintQualiEntries = [prediction.sprint_quali_p1, prediction.sprint_quali_p2, prediction.sprint_quali_p3]
      .map((driverId) => (driverId ? driversById.get(driverId) : null))
      .filter((entry): entry is PodiumEntry => Boolean(entry));
    const sprintRaceEntries = [prediction.sprint_race_p1, prediction.sprint_race_p2, prediction.sprint_race_p3]
      .map((driverId) => (driverId ? driversById.get(driverId) : null))
      .filter((entry): entry is PodiumEntry => Boolean(entry));
    if (sprintQualiEntries.length === 3) {
      sprintQualificationPodium = [sprintQualiEntries[0], sprintQualiEntries[1], sprintQualiEntries[2]];
    }
